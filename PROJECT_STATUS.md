...
- [x] iOS App Store Configurations & Compliance: Configured camera (`NSCameraUsageDescription`), location (`NSLocationWhenInUseUsageDescription`), photo library (`NSPhotoLibraryUsageDescription`, `NSPhotoLibraryAddUsageDescription`), and microphone (`NSMicrophoneUsageDescription`) permission keys inside `Info.plist` (removed unnecessary background/always location keys to prevent configuration mismatches). Registered a standard Apple-compliant Privacy Manifest (`PrivacyInfo.xcprivacy`) inside `project.pbxproj` to declare usage of core system APIs (UserDefaults and File Timestamp). Updated the iOS Bundle Identifier to `com.ahmedmadani.sawascanner`, set deployment target to 15.0 in project/Podfile configurations, and bypassed export compliance.
