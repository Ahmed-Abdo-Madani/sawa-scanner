...
- [x] iOS App Store Configurations: Configured camera (`NSCameraUsageDescription`), location (`NSLocationWhenInUseUsageDescription`, `NSLocationAlwaysAndWhenInUseUsageDescription`, `NSLocationAlwaysUsageDescription`), photo library (`NSPhotoLibraryUsageDescription`, `NSPhotoLibraryAddUsageDescription`), and microphone (`NSMicrophoneUsageDescription`) permission keys inside `Info.plist`. Updated the iOS Bundle Identifier to `com.ahmedmadani.sawascanner` inside `project.pbxproj`, updated the minimum iOS target version to **15.0** inside both `project.pbxproj` and `Podfile` to meet Firebase SDK platform requirements, and added the export compliance bypass key (`ITSAppUsesNonExemptEncryption` = `false`) inside `Info.plist`.
