...
- [x] iOS App Store Configurations: Configured location permission strings (`NSLocationWhenInUseUsageDescription`, `NSLocationAlwaysAndWhenInUseUsageDescription`, and `NSLocationAlwaysUsageDescription`) inside `Info.plist`, updated the iOS Bundle Identifier to your registered ID `com.ahmedmadani.sawascanner` inside `project.pbxproj`, updated the minimum iOS target version to **15.0** inside both `project.pbxproj` and `Podfile` to meet Firebase SDK platform requirements, and added the export compliance bypass key (`ITSAppUsesNonExemptEncryption` = `false`) inside `Info.plist` to prevent export compliance prompts.
