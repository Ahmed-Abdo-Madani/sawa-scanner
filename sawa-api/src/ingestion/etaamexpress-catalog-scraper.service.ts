...
export interface EtaamExpressCatalogStats {
...
@Injectable()
export class EtaamExpressCatalogScraperService {
  private readonly logger = new Logger(EtaamExpressCatalogScraperService.name);
  ...
  constructor(
    ...
    private readonly etaamScraper: EtaamGtinArScraper,
    @InjectQueue('ingestion-queue') private readonly ingestionQueue: Queue,
  ) {}

  async run(opts: EtaamExpressCatalogJobDto): Promise<EtaamExpressCatalogStats> {
    ...
    const progressFilePath = './etaamexpress-scrape-progress.json';
    if (fresh && fs.existsSync(progressFilePath)) {
      try {
        fs.unlinkSync(progressFilePath);
        this.logger.log('[Etaam Express Catalog] Deleted progress tracking file for a fresh run.');
      } catch (err: any) {
        this.logger.warn(`[Etaam Express Catalog] Failed to delete progress file: ${err.message}`);
      }
    }
    ...
    if (fs.existsSync(progressFilePath)) {
      try {
        ...
        this.logger.log(
          `[Etaam Express Catalog] Loaded progress: ${completedCategoryIds.length} categories already processed. Resuming...`,
        );
      } catch (err: any) {
        this.logger.warn(`[Etaam Express Catalog] Failed to read progress file: ${err.message}. Starting fresh.`);
      }
    }

    this.logger.log(
      `[Etaam Express Catalog] Starting catalog scrape: dryRun=${dryRun}, delayMs=${delayMs}, triggerSearch=${triggerSearch}, fresh=${fresh}`,
    );

    // 1. Resolve or create Etaam Express Merchant
    let merchant = await this.merchantRepo.findOne({
      where: { name_en: 'Etaam Express' },
    });
    if (!merchant) {
      this.logger.log('[Etaam Express Catalog] Etaam Express merchant not found, creating it...');
      merchant = this.merchantRepo.create({
        name_en: 'Etaam Express',
        name_ar: 'إطعام إكسبريس',
        base_url: 'https://etaamexpress.com',
    ...
    await this.etaamScraper.ensureLaunched();
    if (!(this.etaamScraper as any).context) {
      throw new Error('Playwright browser context not initialized in Etaam scraper');
    }

    const homePage = await (this.etaamScraper as any).acquirePage();
    let categories: { name: string; url: string; id: string }[] = [];

    try {
      this.logger.log('[Etaam Express Catalog] Discovering categories from home page...');
      await (this.etaamScraper as any).navigateWithEvasion(
        homePage,
        'https://etaamexpress.com/ar',
        'domcontentloaded',
        60000,
        400,
        1200,
      ...
      this.logger.log(`[Etaam Express Catalog] Found ${categories.length} categories on home page.`);
      if (opts.limitCategories && opts.limitCategories > 0) {
        categories = categories.slice(0, opts.limitCategories);
        this.logger.log(`[Etaam Express Catalog] Limited category run to: ${categories.map(c => c.name).join(', ')}`);
      }
      ...
      if (completedCategoryIds.length > 0) {
        ...
        this.logger.log(
          `[Etaam Express Catalog] Resuming scrape: skipped ${initialCount - categories.length} completed categories, ${categories.length} remaining.`,
        );
      }
    } catch (discoveryErr: any) {
      this.logger.error(`[Etaam Express Catalog] Category discovery failed: ${discoveryErr.message}`);
      throw discoveryErr;
    } finally {
      await (this.etaamScraper as any).releasePage(homePage);
    }

    if (categories.length === 0) {
      throw new Error('No categories discovered on Etaam Express. Aborting run.');
    }
    ...
    for (const cat of categories) {
      this.logger.log(`[Etaam Express Catalog] Crawling category: ${cat.name} (ID: ${cat.id})`);
      totalCategoriesProcessed++;

      const page = await (this.etaamScraper as any).acquirePage();
      const catUrl = `https://etaamexpress.com/ar/-/c${cat.id}`;
      ...
      try {
        ...
        while (scrollAttempts < maxScrolls) {
          ...
          if (isButtonVisible) {
            this.logger.log(`[Etaam Express Catalog] Clicking Load More button. Current cards: ${currentCount}`);
        ...
        this.logger.log(`[Etaam Express Catalog] Category ${cat.name}: Finished scrolling. Total cards: ${lastCount}`);
        ...
        for (const p of domProducts) {
          ...
          if (!/^\d{8,14}$/.test(cleanGtin)) {
            this.logger.debug(
              `[Etaam Express Catalog] Barcode not prefixed in URL slug for "${p.name}". Falling back to detail page...`,
            );
            try {
              const details = await this.etaamScraper.scrapeProductDetails(p.url);
              if (details && details.gtin) {
                cleanGtin = details.gtin;
              }
            } catch (err: any) {
              this.logger.warn(`[Etaam Express Catalog] Fallback details scrape failed for ${p.url}: ${err.message}`);
            }
          }

          // Enforce pure numeric EAN check
          if (!/^\d{8,14}$/.test(cleanGtin)) {
            this.logger.warn(`[Etaam Express Catalog] Skipping product "${p.name}" (no valid barcode resolved: ${cleanGtin})`);
            continue;
          }
          ...
          if (isNaN(numericPrice) || numericPrice <= 0) {
            this.logger.warn(`[Etaam Express Catalog] Skipping product "${p.name}" (invalid price: "${p.priceText}")`);
            continue;
          }
          ...
          try {
            await this.dataSource.transaction(async (manager) => {
              ...
              if (!product) {
                product = manager.create(Product, {
                  ...
                  data_source: 'etaam',
                  brand_normalized: normalizeBrandStrict(''), // Salla categories don't expose explicit brand names
            ...
            this.logger.error(`[Etaam Express Catalog] Failed to save products/prices for category ${cat.name}: ${dbErr.message}`);
          }
        }
      } catch (catErr: any) {
        this.logger.error(`[Etaam Express Catalog] Category crawl failed for ${cat.name}: ${catErr.message}`);
      } finally {
        await (this.etaamScraper as any).releasePage(page);
      }
      ...
      if (!dryRun) {
        ...
        try {
          ...
          this.logger.log(`[Etaam Express Catalog] Saved progress after completing category ${cat.name}.`);
        } catch (err: any) {
          this.logger.error(`[Etaam Express Catalog] Failed to write progress file: ${err.message}`);
        }
      }

      // Throttling delay between categories
      if (delayMs > 0 && categories.indexOf(cat) < categories.length - 1) {
    ...
    if (fs.existsSync(progressFilePath)) {
      try {
        fs.unlinkSync(progressFilePath);
        this.logger.log('[Etaam Express Catalog] Scrape finished completely. Cleared progress tracking file.');
      } catch (err: any) {
        this.logger.warn(`[Etaam Express Catalog] Failed to clear progress file: ${err.message}`);
      }
    }

    const durationMs = Date.now() - startTime;
    const stats: EtaamExpressCatalogStats = {
      totalCategoriesProcessed,
      totalProcessed,
      totalAdded,
      totalPricesAdded,
    ...
    this.logger.log(`[Etaam Express Catalog] Finished. Stats: ${JSON.stringify(stats)}`);
    return stats;
  }
}
