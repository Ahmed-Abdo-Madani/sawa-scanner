...
export class NinjaScraper extends BaseScraper {
  ...
  async scrapeListingPage(
    ...
    try {
      ...
      page.on('response', async (response: Response) => {
        if (response.url().includes(this.GRAPHQL_ENDPOINT)) {
          try {
            ...
            if (
              postData.operationName === 'CatalogProducts' ||
              postData.operationName === 'GetCategory'
            ) {
      ...
      dynamicProducts.forEach((p) => {
        if (p.gtin && p.gtin !== '1' && p.productPageUrl)
          capturedProductsMap.set(p.gtin, p);
      });
    ...
    try {
      ...
      const subcategories = await page.evaluate(() => {
        const links = Array.from(
          ...
          (a) =>
            !a.closest(
              'header, nav, footer, aside, [class*="nav"], [class*="header"], [class*="sidebar"], [class*="menu"]',
            ),
        );
        ...
        return uniqueLinks.filter((link) => {
          const isCurrent = link.url.endsWith(window.location.pathname);
          const isRestaurant =
            link.url.includes('/restaurant/') || link.url.includes('cuisine');
    ...
    try {
          ...
          try {
            ...
            if (postData.operationName === 'GetProductDetail') {
              ...
              this.extractProductsFromList(json).forEach((p) => {
                const storefrontId = p.productPageUrl
                  .split('/')
                  .pop()
                  ?.split('-')
                  .pop();
                if (
                  storefrontId === productIdFromUrl ||
                  productUrl.includes(storefrontId || '')
                ) {
      ...
      if (hydration && hydration.length > 0) {
        let matched = hydration.find((p) => {
          const storefrontId =
            p.productPageUrl.split('/').pop()?.split('-').pop() || p.gtin;
          return (
            storefrontId === productIdFromUrl ||
            productUrl.includes(storefrontId || '')
          );
      ...
      if (captured.size === 0) {
        this.logger.debug(
          `Hydration/GQL extraction failed for ${productUrl}. Attempting DOM fallback...`,
        );
        const domData = await page.evaluate((url) => {
          ...
          return {
            ...
            description:
              document
                .querySelector('[class*="description"], [class*="Description"]')
                ?.textContent?.trim() || '',
          };
        }, productUrl);
      ...
      const best = products.sort(
        (a: ScrapedProductData, b: ScrapedProductData) =>
  ...
  private async sweepHydrationData(
    page: Page,
    filterPath?: string,
  ): Promise<ScrapedProductData[]> {
    ...
    for (const { id, type, text } of scriptContents) {
      ...
      if (id === '__NEXT_DATA__' || text.includes('"pageProps"')) {
        try {
          ...
          if (json) {
            this.extractProductsFromList(json, filterPath).forEach((p) =>
              products.push(p),
            );
          }
        } catch (e) {}
      ...
      if (text.includes('self.__next_f.push')) {
        ...
        while ((match = regexNextF.exec(text)) !== null) {
          try {
            const rawEncoded = match.groups.content;
            const raw = rawEncoded
              .replace(/\\"/g, '"')
              .replace(/\\\\/g, '\\')
              .replace(/\\n/g, '\n')
              .replace(/\\t/g, '\t');
        ...
        const jsons = [
          ...findJSONObjects(fullStream, '"productId"'),
          ...findJSONObjects(fullStream, '"gtin"'),
        ];
        jsons.forEach((json) => {
  ...
  private extractProductsFromList(
    json: any,
    filterPath?: string,
  ): ScrapedProductData[] {
    const products: ScrapedProductData[] = [];
    ...
    const recursiveFind = (obj: any) => {
      ...
      const hasProductKeys =
        obj.price !== undefined ||
        obj.priceCents !== undefined ||
        obj.isAvailable !== undefined ||
        obj.inStock !== undefined ||
        obj.gtin ||
        obj.offers;
  ...
  private mapProduct(p: any): ScrapedProductData | null {
    ...
    if (
      ...
      lowerName.includes('deal of the day')
    ) {
      return null;
    }
    ...
    const priceCents =
      p.priceCents || p.discountedPriceCents || p.originalPriceCents || 0;
    if (priceCents > 0) {
    ...
    // 3. Fallback to raw price strings/objects
      ...
      const rawPrice =
        p.price?.amount || p.price || p.currentPrice || p.current_price || '0';
      price =
    ...
    if (
      !productPageUrl ||
      !productPageUrl.includes(`/product/${storefrontId}`)
    ) {
      const base =
        slug.includes('/pharmacy/') || p.isPharmacy
          ? '/sa/en/pharmacy/product/'
          : '/sa/en/product/';
    ...
    const imageUrls =
      p.imageUrls ||
      [
        ...asArray(p.medias).map(
          (m: any) => m.url || m.path || (typeof m === 'string' ? m : ''),
        ),
        ...asArray(p.images).map(
          (img: any) =>
            img.url || img.path || (typeof img === 'string' ? img : ''),
        ),
        ...(p.image
          ? [
