...
export class OthaimPriceScraper extends BaseScraper {
  ...
  async scrapeProductPrice(productUrl: string): Promise<{ price: number; inStock: boolean }> {
    ...
    try {
      ...
      const priceSelector = '.product-price, .price-box';
      await page.waitForSelector(priceSelector, { timeout: 15000 });

      const data = await page.evaluate(() => {
        const pEl = document.querySelector('.product-price, .price-box');
        const priceText = pEl?.textContent || '0';
        const outOfStock = document.body.innerText.includes('Out of stock') || 
                           document.body.innerText.includes('نفذت الكمية');
