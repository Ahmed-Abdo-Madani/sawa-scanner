...
export class OthaimPriceScraper extends BaseScraper {
  async scrapeListingPage(
    ...
    try {
      ...
      const productCardSelector =
        'a[class*="productBoxLink"], div[data-qa="product-grid-item"]';
        ...
        return cards
          .map((card) => {
            const nameEl = card.querySelector(
              'div[data-qa="product-name"], [class*="productTitle"]',
            );
  ...
  async scrapeDetailPage(
    ...
    try {
      ...
      await page.waitForSelector('h1[data-qa="product-name"], .productName', {
        timeout: 30000,
      });

      const detail = await page.evaluate(() => {
        const nameEl = document.querySelector(
          'h1[data-qa="product-name"], .productName',
        );
        ...
        const imgElements = Array.from(
          document.querySelectorAll(
            '.imageContainer img, [data-qa="product-image"]',
          ),
        );
