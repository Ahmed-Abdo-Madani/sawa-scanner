...
export class PandaPriceScraper extends BaseScraper {
  async scrapeListingPage(categoryUrl: string, pageNum: number): Promise<ScrapedProductData[]> {
    ...
    try {
      ...
      const productCardSelector = 'div.relative.flex.flex-col.h-full, .product-item';
      await page.waitForSelector(productCardSelector, { timeout: 30000 });

      const products = await page.evaluate((selector) => {
        const cards = Array.from(document.querySelectorAll(selector));
        return cards.map((card) => {
          const nameEl = card.querySelector('span.text-black-700.text-sm.font-bold, span[class*="productName"], .name');
          const priceEl = card.querySelector('span.text-primary.text-lg.font-bold, .price, [class*="price"]');
  ...
  async scrapeDetailPage(productUrl: string): Promise<ScrapedProductData & { page?: Page }> {
    ...
    try {
      ...
      await page.waitForSelector('h1, [class*="productTitle"]', { timeout: 30000 });

      const detail = await page.evaluate(() => {
        const nameEl = document.querySelector('h1, [class*="productTitle"]');
        const priceEl = document.querySelector('.price, [class*="productPrice"], span.text-primary');
        const brandEl = document.querySelector('.brand, [class*="brandName"]');
        const weightEl = document.querySelector('.weight, [class*="productWeight"]');
        
        const imgElements = Array.from(document.querySelectorAll('img[class*="productImage"], .gallery img'));
