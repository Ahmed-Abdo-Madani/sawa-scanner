...
export class CarrefourPriceScraper extends BaseScraper {
      ...
      const products = await page.evaluate((selector) => {
        ...
        return cards
          .map((card) => {
            ...
            const nameEl =
              card.querySelector('[data-testid="product_name"]') ||
  ...
  async scrapeDetailPage(
    ...
    try {
      ...
      await page.waitForSelector('h1, [data-testid="product_name"]', {
        timeout: 30000,
      });
      ...
      const detail = await page.evaluate(() => {
        ...
        const descEl = document.querySelector(
          '.text-sm.leading-6, #description',
        );

        const imgElements = Array.from(
          document.querySelectorAll(
            'img[data-testid="product_image"], .gallery img',
          ),
        );
        ...
        try {
          ...
          for (const script of ldScripts) {
            ...
            if (
              product &&
              (product.gtin13 || product.sku || product.gtin12 || product.isbn)
            ) {
              gtin =
                product.gtin13 || product.sku || product.gtin12 || product.isbn;
              break;
