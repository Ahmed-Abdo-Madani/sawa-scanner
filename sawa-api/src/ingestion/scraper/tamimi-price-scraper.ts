...
export class TamimiPriceScraper extends BaseScraper {
  ...
  async scrapeDetailPage(
    ...
    try {
      ...
      await page.waitForSelector('h1, [class*="ProductDetail__Name"]', {
        timeout: 30000,
      });

      const detail = await page.evaluate(() => {
        const nameEl = document.querySelector(
          'h1, [class*="ProductDetail__Name"]',
        );
        const priceEl = document.querySelector(
          'span[class*="ProductDetail__Price"]',
        );
        const brandEl = document.querySelector(
          '[class*="ProductDetail__Brand"]',
        );
        const descEl = document.querySelector(
          '[class*="ProductDetail__Description"]',
        );
        const weightEl = document.querySelector(
          '[class*="ProductDetail__Weight"]',
        );

        const imgElements = Array.from(
          document.querySelectorAll('[class*="ProductDetail__Image"] img'),
        );
        ...
        const sEl = document.querySelector(
          '.stock.available, .in-stock, [class*="InStock"]',
        );
        const inStock =
          !!sEl ||
