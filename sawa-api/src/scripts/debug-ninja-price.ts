...
async function run() {
        ...
        if (response.url().includes('graphql.ananinja.com')) {
            try {
                ...
                const recursiveFind = (obj: any) => {
                    ...
                    if (obj.productId === '19304137' || obj.id === '19304137' || obj.gtin === '19304137') {
                        productData = obj;
                    }
                    if (!productData) {
    ...
    await page.goto('https://ananinja.com/sa/en/product/19304137', { waitUntil: 'networkidle' });
    ...
    if (productData) {
        ...
        for (const content of scripts) {
            if (content?.includes('19304137')) {
                ...
                const chunk = content.substring(content.indexOf('19304137') - 500, content.indexOf('19304137') + 2000);
                console.log(chunk);
            }
        }
    }
