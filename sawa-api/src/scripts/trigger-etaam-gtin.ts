...
async function triggerEtaamGtinScrape() {
  console.log('🛒 Triggering Etaam Express GTIN Enrichment Job...');
  ...
  try {
    ...
    const res = await axios.post(
      `${API_BASE_URL}/ingestion/etaam-gtin`,
      parsedFlags,
      {
        headers: {
    ...
    console.log('✅ Etaam GTIN Enrichment job triggered successfully!');
    console.log(`🚀 Enqueued: ${enqueued} products`);
    console.log(`⚠️  Skipped:  ${skipped} products`);
    console.log(
      '\n📊 Monitor the Bull Board at http://localhost:3000/admin/queues to track progress.',
    );
  } catch (error: any) {
    console.error('❌ Failed to trigger Etaam GTIN Enrichment:');
    console.error(
      'Error:',
