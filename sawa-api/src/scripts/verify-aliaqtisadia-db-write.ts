...
async function bootstrap() {
  console.log('🛒 Bootstrapping Minimal NestJS Context for Aliaqtisadia Verification (No Redis)...');
  ...
  try {
    ...
    console.log('\n============================================================');
    console.log('📊 ALIAQTISADIA STORE SCRAPER STATS:');
    console.log('============================================================');
    ...
    // 1. Verify DB Seeding
    console.log('🔍 Querying DB for newly seeded Aliaqtisadia products...');
    const seededProducts = await productRepo.find({
      where: { data_source: 'aliaqtisadia' },
      relations: ['prices'],
      take: 10,
    });

    console.log(`Found ${seededProducts.length} Aliaqtisadia products in database:`);
