...
async function triggerDukanExpressCatalogScrape() {
  console.log('🏪 Triggering Dukan Express Catalog Scrape...');
  ...
  try {
    ...
    const res = await axios.post(
      `${API_BASE_URL}/ingestion/dukanexpress-catalog-scrape`,
      parsedFlags,
      {
        headers: {
    ...
    if (res.status === 409 || !created) {
      console.warn('⚠️  Dukan Express catalog scrape not queued (conflict):');
      console.warn('Message:', message || 'A scrape is already in progress.');
      process.exit(0);
    }

    console.log('✅ Dukan Express Catalog Scrape job triggered successfully!');
    ...
    console.error('❌ Failed to trigger Dukan Express catalog scrape:');
    console.error(
      'Error:',
...
triggerDukanExpressCatalogScrape();
