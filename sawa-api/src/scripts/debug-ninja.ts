...
async function debugNinja() {
  const scratchDir = 'C:/Users/Design_Bench_12/.gemini/antigravity/brain/910d92a9-7e36-45e5-9843-af2749d98c2a/scratch';
  ...
  page.on('response', async res => {
    if (res.url().includes('graphql') || res.url().includes('api')) {
       ...
       try {
         ...
         fs.writeFileSync(path.join(scratchDir, `res-${Date.now()}.json`), JSON.stringify({
            url: res.url(),
            status: res.status(),
  ...
  try {
    ...
    await page.screenshot({ path: path.join(scratchDir, 'ninja-debug.png'), fullPage: true });
