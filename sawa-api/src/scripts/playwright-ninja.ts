  ...
  for (const content of scripts) {
      if (content.includes('self.__next_f.push')) {
        ...
        while ((match = regexNextF.exec(content)) !== null) {
          try {
             let unescaped = match[1].replace(/\\"/g, '"').replace(/\\\\/g, '\\').replace(/\\n/g, '');
...
function findJSONObjects(text: string, marker: string): any[] {
  ...
  while ((searchIndex = text.indexOf(marker, searchIndex)) !== -1) {
    let openBraceIndex = text.lastIndexOf('{', searchIndex);
    if (openBraceIndex === -1) { searchIndex++; continue; }
    let braceCount = 0;
    let inString = false;
    let isEscaped = false;
    let closedIndex = -1;
    ...
    if (closedIndex !== -1) {
        ...
        try { results.push(JSON.parse(potentialJson)); } catch (e) {}
    }
