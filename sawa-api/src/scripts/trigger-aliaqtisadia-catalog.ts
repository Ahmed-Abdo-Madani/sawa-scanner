...
async function triggerAliaqtisadiaCatalogScrape() {
  console.log('🏪 Triggering Aliaqtisadia Catalog Scrape...');

  // Parse command-line arguments
  const args = process.argv.slice(2);
  const parsedFlags: any = {
    dryRun: false,
    triggerOtherStoresSearch: false, // Default is false as requested
  };
  ...
  try {
    ...
    const res = await axios.post(
      `${API_BASE_URL}/ingestion/aliaqtisadia-catalog-scrape`,
      parsedFlags,
      {
        headers: {
    ...
    if (res.status === 409 || !created) {
      console.warn('⚠️  Aliaqtisadia catalog scrape not queued (conflict):');
      console.warn('Message:', message || 'A scrape is already in progress.');
      process.exit(0);
    }

    console.log('✅ Aliaqtisadia Catalog Scrape job triggered successfully!');
    ...
    console.error('❌ Failed to trigger Aliaqtisadia catalog scrape:');
    console.error(
      'Error:',
...
triggerAliaqtisadiaCatalogScrape();
