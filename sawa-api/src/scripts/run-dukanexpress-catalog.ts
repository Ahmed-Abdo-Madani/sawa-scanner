...
async function bootstrap() {
  console.log('🚀 Bootstrapping NestJS Context for Full Dukan Express Catalog Scrape...');
  ...
  try {
    ...
    console.log('Progress will be saved locally to "./dukanexpress-scrape-progress.json"\n');

    const stats = await service.run({
      dryRun: false,
      delayMs: 300, // Safe delay between page hits to avoid triggering rate limits
      triggerOtherStoresSearch: false, // Bypassing other store enqueues remains strict
      fresh,
    });
