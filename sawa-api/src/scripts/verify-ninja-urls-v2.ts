  ...
  for (const id of testIds) {
    ...
    try {
      ...
      if (res.status === 200 && !res.data.includes('Product not found')) {
        ...
        if (resPharm.status === 200 && !resPharm.data.includes('Product not found')) {
             console.log(`✅ ${pharmacyUrl} is working (Status ${resPharm.status})`);
        } else {
             console.log(`❌ ${url} and ${pharmacyUrl} failed (Status ${res.status}/${resPharm.status})`);
        }
      }
    } catch (err: any) {
