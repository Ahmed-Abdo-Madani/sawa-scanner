...
async function triggerEtaamExpressCatalogScrape() {
  console.log('🏪 Triggering Etaam Express Catalog Scrape...');
  ...
  try {
    ...
    const res = await axios.post(
      `${API_BASE_URL}/ingestion/etaamexpress-catalog-scrape`,
      parsedFlags,
      {
        headers: {
    ...
    if (res.status === 409 || !created) {
      console.warn('⚠️  Etaam Express catalog scrape not queued (conflict):');
      console.warn('Message:', message || 'A scrape is already in progress.');
      process.exit(0);
    }

    console.log('✅ Etaam Express Catalog Scrape job triggered successfully!');
    ...
    console.error('❌ Failed to trigger Etaam Express catalog scrape:');
    console.error(
      'Error:',
...
triggerEtaamExpressCatalogScrape();
