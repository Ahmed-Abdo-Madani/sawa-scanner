...
async function triggerJobs() {
  console.log(`🚀 Starting Ninja-only wave of ${categories.length} ingestion jobs...`);
  
  for (const cat of categories) {
    try {
      console.log(`Triggering ${cat.platform}: ${cat.categoryUrl} (Pages ${cat.pageRange.start}-${cat.pageRange.end})...`);
      ...
      console.error(`❌ Failed ${cat.platform}: ${error.response?.data?.message || error.message}`);
    }
  }
