...
async function triggerYasminCatalogScrape() {
  console.log('🏪 Triggering Yasmin Store Catalog Scrape...');
  ...
  try {
    ...
    const res = await axios.post(
      `${API_BASE_URL}/ingestion/yasmin-catalog-scrape`,
      parsedFlags,
      {
        headers: {
    ...
    if (res.status === 409 || !created) {
      console.warn('⚠️  Yasmin Store catalog scrape not queued (conflict):');
      console.warn('Message:', message || 'A scrape is already in progress.');
      process.exit(0);
    }

    console.log('✅ Yasmin Store Catalog Scrape job triggered successfully!');
    ...
    console.error('❌ Failed to trigger Yasmin Store catalog scrape:');
    console.error(
      'Error:',
