...
async function test() {
  try {
    ...
    const matches = Array.from(body.matchAll(/self\.__next_f\.push\(\[1,"(.*?\\])"\]\)/g));
    let assembled = '';
    ...
    if (idIdx > -1) {
      console.log('Context of productId:', body.substring(Math.max(0, idIdx - 100), idIdx + 300));
    } else {
      const escapedIdIdx = body.indexOf('\\"productId\\"');
      if (escapedIdIdx > -1) {
        console.log('Context of \\"productId\\":', body.substring(Math.max(0, escapedIdIdx - 100), escapedIdIdx + 500));
      } else {
