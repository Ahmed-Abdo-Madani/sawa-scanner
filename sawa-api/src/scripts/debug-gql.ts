  ...
  page.on('response', async response => {
    if (response.url().includes('graphql')) {
      try {
        ...
        console.log(`[GQL RES] Operations: ${Object.keys(json.data || {}).join(', ')}`);
        // If it looks like products, dump partial JSON
        const productsKey = Object.keys(json.data || {}).find(k => k.toLowerCase().includes('product') || k.toLowerCase().includes('item') || k.toLowerCase().includes('catalog'));
        if (productsKey) {
          console.log(`[GQL DATA] Found likely products in key: ${productsKey}`);
